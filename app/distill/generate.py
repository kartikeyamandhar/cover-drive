...
def run_batch(
    ...
    *,
    ...
    """Generate via the Batches API (50% off). Warms the cache, submits, collects."""
    manifest = DistillManifest.load(config.manifest_path)
    seeds = seed_lines()
    stats = RunStats()
    ...
    by_custom_id: dict[str, WorkItem] = {}
    ...
    log.info("batch submitted", batch_id=batch.id, n=len(requests))
