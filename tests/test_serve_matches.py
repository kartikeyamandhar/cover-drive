...
def test_allowed_ids_filters_the_listing_but_not_replay(tmp_path: Path) -> None:
    _write_match(tmp_path, "1001")
    _write_match(tmp_path, "1002", balls=4)
    repo = MatchRepository(tmp_path, allowed_ids=("1001",))
    assert [s.match_id for s in repo.list_matches()] == ["1001"]  # listing is filtered
    assert len(repo.load_balls("1002")) == 4  # but any on-disk match is still replayable
...
def test_max_listed_caps_discovery_but_not_replay(tmp_path: Path) -> None:
    for i in range(5):
        _write_match(tmp_path, f"200{i}")
    repo = MatchRepository(tmp_path, max_listed=2)
    listed = [s.match_id for s in repo.list_matches()]
    assert listed == ["2000", "2001"]  # listing is capped
    assert len(repo.load_balls("2004")) == 3  # but any on-disk match is still replayable
